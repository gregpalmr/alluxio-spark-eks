...
- Change the references to the AWS region and availability zones. Change us-west-1 and us-west-1a and us-west-1b as needed. 
- Add a reference to your private SSH key, if you want to be able to SSH to the EC2 instances. Replace PUT_YOUR_PATH_TO_PUB_SSH_KEY_HERE with the path to your public ssh key. 
- Optionally change the EC2 instance type, but make sure you choose one that has enough cpu vcores, memory and NVMe storage to support running both Spark pods and Alluxio pods and that allow Alluxio to cache enough data on NVMe storage to improve performance.
- Change the number of nodes in your EKS cluster to support the workloads you are running. For Alluxio, you will require a minimum of 3 nodes.
